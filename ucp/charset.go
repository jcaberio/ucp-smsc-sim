...
func decodeIRA(msg []byte) string {
	var output string
	for start, end := 0, 1; end <= len(msg); start, end = start+2, end+2 {
		str := string(msg[start : end+1])
		i, _ := strconv.ParseInt(str, 16, 32)
		output += string(gsmTable[i])
	}
	return output
}
